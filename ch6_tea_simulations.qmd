...
by_condition <- tea_data %>%
  ...
  summarize(mean_rating = mean(rating),
            num_participants = length(rating))
...
This histogram looks like a normal distribution because of high n.
...
ggplot(data = tea_data, aes(x=condition, y=rating, col=condition))  +
  ...
  theme_few() +
  geom_pointrange(data = tea_data_summary_cis, aes(x=condition, y = cond_mean, ymin = ci_lower, ymax = ci_upper)) +
  ylim(0,10) +
...
ggplot(data = tea_data_highn, aes(x=condition, y=rating, col=condition))  +
  ...
  theme_few() +
  geom_pointrange(data = tea_data_summary_cis, aes(x=condition, y = cond_mean, ymin = se_lower, ymax = se_upper)) +
  ylim(0,10) +
...
Each dot is one experiment. The darker, large dots are mean. The CI means we are 95% confident in the mean and the difference we observe is true. The SE indicates the variability in the mean.
